...
print "Okay running density2d_thickness...."
...
try:
	...
	for idx in range(len(ST_trajectoryFile)):
		...
		if (cnt % ST_frmInt) == 0:
			...
			for i in range(len(top_Res)):
			    ...
			    # calculating the cooridnates of z axis
			    if Tsel.numberOfAtoms() > 1:
				x = np.mean(Tsel.coordinates()[:,0]);
				y = np.mean(Tsel.coordinates()[:,1]);
				z = np.mean(Tsel.coordinates()[:,2]);
			    else:
				x = Tsel.coordinates()[:,0];
				y = Tsel.coordinates()[:,1];
				z = Tsel.coordinates()[:,2];
			...
			for i in range(len(btm_Res)):
			    ...
			    Bsel = u.selectAtoms(btm_Qry);
			    # calculating the cooridnates of z axis
			    if Bsel.numberOfAtoms() > 1:
				x = np.mean(Bsel.coordinates()[:,0]);
				y = np.mean(Bsel.coordinates()[:,1]);
				z = np.mean(Bsel.coordinates()[:,2]);
			    else:
				x = Bsel.coordinates()[:,0];
				y = Bsel.coordinates()[:,1];
				z = Bsel.coordinates()[:,2];
			    # finding associated BIN
	...
	fboth = '{0}/both_{1}'.format(ST_out_dir, ST_outFile);
	fid_fboth = open(fboth, 'w');
	...
	for i in range(len(BIN_top)):
		...
		out_data_both = "{}\t{}\t{}\n".format(BIN_btm[i,0], BIN_btm[i,1], BIN_top[i,2] + BIN_btm[i,2]);
	        fid_fboth.write(out_data_both);
	...
	gScript = gScript + "set multiplot layout 3, 1 title '2D thickness density profile'\n";
	
	gScript = gScript + """set xtics offset 0,0.5; unset xlabel\n""";
	gScript = gScript + """set label 1 'Top' at graph 0.01, 0.95 font ',8'\n""";
	gScript = gScript + """set ylabel 'Y axis' offset 0,-8\n""";
	...
	gScript = gScript + """set xtics offset 0,0.5; unset xlabel; unset ylabel\n""";
	gScript = gScript + """set label 1 'Bilayer' at graph 0.01, 0.95 font ',8'\n""";
	gScript = gScript + """set xlabel 'X axis' offset 0,1\n""";
	gScript = gScript + """set pm3d map\n""";
	gScript = gScript + """set palette defined (0 "white", {} "blue", {} "green", {} "red")\n""".format(topIntv[1], topIntv[2], topIntv[3]);
	gScript = gScript + """set cbrange [0:{}]\n""".format(top_max);
	gScript = gScript + """set cbtic {}\n""".format(top_intv);
