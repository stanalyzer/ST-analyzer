...
try:
	...
	fid_out.write("# Trajectory\tTime(ps)\tDensity\n");
	...
	for idx in range(len(trajectoryFile)):
	    ...
	    # initializing density array == 0; the length is same as BIN
	    frmDNST = [];	# based on time frame
	    frmSTMP = [];	# based on time frame
	    for ibin in frange(dnst_min, dnst_max, dnst_bin):
		frmDNST.append(0.0);
    
	    # read based on frame
	    for ts in u.trajectory:
		timeDNST = [];	# based on time frame
		timeSTMP = [];	# based on time frame
		for ibin in frange(dnst_min, dnst_max, dnst_bin):
		    timeDNST.append(0);
		...
		if (cnt % frmInt) == 0:
			...
			cmt = "{0}\t{1}".format(idx, tmp_time);
			fid_out.write(cmt);
			for key in intKEY:
			    idx_bin = BIN.index(key);
			    timeDNST[idx_bin] = float(tmpDNST[key]) / float(bin_vol); # normalizing based on volume
			
			print timeDNST;
			
			for curr_dnst in timeDNST:
			    cmt = "\t{0}".format(curr_dnst)
			    fid_out.write(cmt);
			fid_out.write("\n");
			
			frmDNST.append(timeDNST);
			frmSTMP.append(timeSTMP);
			...
			for curr_dnst in timeDNST:
			    ...
			    fid_out.write(cmt);
			fid_out.write("\n");
			frmDNST.append(timeDNST); # set zero
			frmSTMP.append(timeSTMP);
			
	    # accumulating them into trajectory based array
	    trjDNST.append(frmDNST);
	    trjSTMP.append(frmSTMP);
	    #print "*** total {0}trajectory, {1}frames, and {2}bins".format(len(trjDNST), len(trjDNST[0]), len(trjDNST[0][0]));
	fid_out.close()
	...
	gScript = gScript + """plot "{0}/{1}" using 2:3 title "Density" with linespoints\n""".format(out_dir, outFile);
