...
alig_sele    = str(paras[3][para_idx]);			
calc_sele    = str(paras[4][para_idx]);

#"True":calculate average on residue basis
...
try:
	...
	if res_ave == "False":
	    fid_out.write("# AtomIndex\tRMSF\tAtomName\tresname\tsegid\tresnum\n")
	else:
	    fid_out.write("# ResIndex\tRMSF\tresname\tsegid\tresnum\n")
    
	# Writes comments in the output file
	fid_out.write("# Time(ps)\tRMSD(A)\n")
	fid_out.write("# Alignment selection: %s\n"%alig_sele)
	fid_out.write("# RMSF calculation selection: %s\n"%calc_sele)
	now = datetime.now()
	fid_out.write("# Created on: %s\n"%str(now))
	
	psf = '{0}{1}'.format(base_path, structure_file);
	
	cnt    = 0;	# number of entire frame
	ncount = 0; # number of counted frame based on frame interval
	
	ref = Universe(base_path+"/"+pdb_file);
	
	rmsf_atom = ref.selectAtoms(calc_sele);
	cor1 = rmsf_atom.get_positions();
	cor_sqar_ave = np.zeros((len(cor1), 3), dtype=np.float);
	cor_ave = np.zeros((len(cor1), 3), dtype=np.float);
	...
	for idx in range(len(trajectoryFile)):
	    ...
	    rmsf_atom = u.selectAtoms(calc_sele);
		...
		if (cnt % frmInt) == 0:
		    ...
		    alignto(u, ref, select=alig_sele, subselection=calc_sele);
		    cor1 = rmsf_atom.get_positions();
		    cor_ave = cor_ave + cor1;
		    cor_sqar_ave = cor_sqar_ave + cor1*cor1;
		    ncount = ncount + 1;
    
	cor_ave = cor_ave/float(ncount);
	cor_sqar_ave = cor_sqar_ave/float(ncount);
	rmsf = np.sqrt(np.sum(cor_sqar_ave - cor_ave*cor_ave, axis=1));
	names    = rmsf_atom.names();
	resnames = rmsf_atom.resnames();
	resnums  = rmsf_atom.resnums();
    
	# average based on each atom
	if res_ave == "False":
	    for i in range(0, len(cor1)):
		fid_out.write("%d %f %s %s %s %d\n"%(i+1, rmsf[i], names[i],\
			rmsf_atom._atoms[i].resname, rmsf_atom._atoms[i].segid, \
			rmsf_atom._atoms[i].resnum));
	else:
	    # average on residue
	    reslist = [];
	    atomgroup = [];
	    ...
	    for i in range(0, len(cor1)):
		resname = rmsf_atom._atoms[i].resname;
		segid = rmsf_atom._atoms[i].segid;
		resnum = str(rmsf_atom._atoms[i].resnum);
		com = "-".join([resname, segid, resnum]);
		if com in reslist:
		    k = reslist.index(com);
		    atomgroup[k].append(i);
		else:
		    reslist.append(com);
		    atomgroup.append([i]);
		    
	    for i in range(0, len(reslist)):
		resname, segid, resnum = reslist[i].split("-");
		rmsff = 0;
		for k in atomgroup[i]:
		    rmsff = rmsff + rmsf[k];
		rmsff = rmsff/len(atomgroup[i]);
		fid_out.write("%d %f %s %s %s\n"%(i+1, rmsff, resname, segid, resnum));
	...
	if res_ave == "False":
	    gScript = gScript + "set xlabel 'Atom Index'\n";
	else:
	    gScript = gScript + "set xlabel 'Residue Index'\n";
	    
	gScript = gScript + "set ylabel 'RMSF (Angstrom)'\n";
	
	gScript = gScript + "set output '{0}'\n".format(imgPath);
	gScript = gScript + """plot "{0}/{1}" using 1:2 title "RMSF" with lines lw 3\n""".format(out_dir, outFile);
