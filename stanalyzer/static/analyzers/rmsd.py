...
try:
	...
	fid_out.write("# RMSD calculation selection: %s\n"%calc_sele)
	now = datetime.now()
	...
	for idx in range(len(trajectoryFile)):
		...
		if (cnt % frmInt) == 0:
		    ...
		    b = alignto(u, ref, select=alig_sele, subselection=calc_sele)
		    fid_out.write("%d %f\n"%(tmp_time, b[1]))
	...
	gScript = gScript + "set ylabel 'RMSD (Angstrom)'\n";
	gScript = gScript + "set output '{0}'\n".format(imgPath);
	gScript = gScript + """plot "{0}/{1}" using 1:2 title "RMSD" with lines lw 3\n""".format(out_dir, outFile);
